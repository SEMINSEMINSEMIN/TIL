...
```
제거한 요소들이 있는 배열을 반환한다. 제거한 요소가 없을 경우 빈 배열을 반환한다.
```javascript
...
```
## 8.13. map(): 특정 조건에 부합하는 요소들로 이루어진 배열 반환