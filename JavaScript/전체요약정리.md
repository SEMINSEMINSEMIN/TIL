...
* [웹 어셈블리](https://developer.mozilla.org/ko/docs/WebAssembly/Concepts)
...
```
```js
// 주의해야할 점: 변수 이름이 중복됐을 때 -> 에러가 날 수 있음
...
* [JS 난독화](https://obfuscator.io/): 내 코드 보호
4. 주석
```js
// 한 줄 주석입니다.
/*
...
![](/images/%ED%95%A8%EC%88%98%EC%B0%B8%EC%A1%B0%ED%83%80%EC%9E%85.png)
...
- Number(숫자)
    ...
    * Math
        * Math.PI
        * Math.max(): 그냥 출력하면 -Infinity
        * Math.min(): 그냥 출력하면 Infinity
        * Math.floor()
        * Math.round()
        * Math.random()
        * Math.abs()
...
- String(문자열)
    ...
    * 메서드 : 
        ...
        * str.repeat()
        ...
        '5'.repeat(100).split('').map(parseInt);
        // 중간에 NaN이 됩니다. 2번째 인자가 진수여서 그럼
...
- Array(배열) : object
    ...
    * 메서드 : 
        ...
        * map
        * filter
        * push / pop - mutable(원본 배열 수정)
        * slice - immutable(원본 배열 수정 x)
        * splice - mutable
        * reduce - immutable
        * join
        * indexOf
...
- 화살표함수
    ...
    // 함수표현식, 호이스팅 x (호이스팅 되지만 사용할 수 없다)
    let 제곱 = x => x**2;
...
## 사용자와 상호작용
* prompt('hello'): prompt로 얻어진 값은 모두 문자열이다!
* comfirm('hello')
* alert('hello')
